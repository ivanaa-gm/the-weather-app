...
const HourlyWeatherCard = ({
  ...
  return (
    <div
      ...
      className={
        isToday
          ? "backdrop-blur-sm flex flex-col items-center min-w-[140px] gap-2 border-2 border-black/10 shadow-xl bg-white/30 rounded-xl p-2 m-1 text-sm transition duration-400 ease-in-out transform hover:ring-1 hover:ring-white/30 hover:ring-offset-0 hover:shadow-white/40"
    ...
    >
      ...
      <img
        ...
        title={t(`weatherCodes.${weatherCode}`)}
        className={isToday ? "h-16" : "h-12"}
      />
      <p
        className={
          isToday
            ? "text-2xl font-poiret font-semibold"
            : "text-xl font-poiret font-semibold"
        }
      >
      ...
      <div className="flex flex-row items-center">
        ...
        <p className={isToday ? "text-lg" : "text-sm"}>
          {windSpeed} {t(`${metrics.windSpeed}`)}{" "}
          {t(`directions.${windDirectionString}`)}
        </p>
      </div>
    </div>
  );
